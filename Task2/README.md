...
Диаграмма выполнена на уровне C4 Context. Мобильное приложение показано как software system, а пациент — как person, который использует его. Пунктирная граница «Здоровье+ — внутренний ИТ-ландшафт» означает границу владения организации: внутри неё находятся новые платформы и сохраняемые внутренние системы, включая CRM, 1С: Полисы, LIS и Расписание; платёжный шлюз и провайдеры сообщений остаются внешними. Название External API Gateway означает публичную точку входа, а не внешнее владение. Внутренние базы данных, очереди и отдельные микросервисы намеренно не вынесены на диаграмму: это детали C4 Container и последующих заданий.
...
- мобильное приложение работает через внешний API Gateway/WAF и CIAM, не обращаясь к legacy-системам напрямую;
- врачи, администраторы и лаборанты работают через отдельный Internal Gateway/Access Proxy и корпоративный IAM/SSO;
- интеграционная платформа изолирует контракты и сбои legacy-систем, а веб-портал перестаёт быть интеграционным центром.
...
| `ARCH-S-003` | External API Gateway / WAF | Публичный защищённый вход мобильного приложения, проверка токена, маршрутизация и политики API | `ARCH-I-001`, `ARCH-I-004`, `ARCH-I-011` |
...
| `ARCH-S-008` | Корпоративный IAM / SSO | Единая аутентификация сотрудников, MFA, жизненный цикл учётных записей и передача ролей | `ARCH-I-011` |
| `ARCH-S-009` | Internal Gateway / Access Proxy | Единый внутренний вход для веб-портала и LIS, проверка корпоративной идентичности, RBAC и маршрутизация к внутренним системам | `ARCH-I-001`, `ARCH-I-011` |
...
| `ARCH-REL-002` | Мобильное приложение → External API Gateway/WAF: запросы записи, оплаты, анализов и ДМС | Синхронно, HTTPS/REST | API Gateway | Даёт публичному мобильному каналу единую точку входа без прямого доступа к CRM, 1С, LIS и расписанию | `ARCH-I-001`, `ARCH-I-011` |
| `ARCH-REL-003` | External API Gateway/WAF: локальная проверка access token; CIAM предоставляет JWKS для периодического обновления кеша | Локальная криптографическая проверка JWT без сетевого вызова на каждый запрос; JWKS обновляется отдельно по TTL и при неизвестном `kid` с защитой от stampede | Token Validation, Cache-Aside | CIAM не входит в синхронный путь каждого API-запроса; кратковременная недоступность CIAM не останавливает проверку токенов, пока кешированные ключи валидны | `ARCH-I-002`, `ARCH-I-005`, `ARCH-I-011` |
| `ARCH-REL-004` | External API Gateway/WAF → цифровая платформа пациентов: версионированный Mobile API | Синхронно, HTTPS/REST | Backend for Frontend, API-led Integration | BFF возвращает данные в форме мобильного UI и не раскрывает внутренние контракты backend-систем | `ARCH-I-001`, `ARCH-I-010` |
...
| `ARCH-REL-011` | Интеграционная платформа → CRM: профиль пациента и результаты, включая результаты LIS после миграции legacy-потока | Синхронно, REST через адаптер; прямой LIS → CRM временно сохраняется только на переходе | Anti-Corruption Layer, Strangler Fig, Request/Response | Целевой путь LIS → интеграционная платформа → CRM заменяет прямую legacy-интеграцию после сверки полноты, порядка и дедупликации результатов; мобильный контур не видит контракт CRM | `ARCH-I-001`, `ARCH-I-007`, `ARCH-I-010` |
| `ARCH-REL-012` | Интеграционная платформа ↔ 1С: Полисы: проверка ДМС и контролируемая синхронизация | Ограниченные синхронные HTTP-вызовы к системе-источнику; результат публикуется через `ARCH-REL-008`, а проекция ДМС обновляется асинхронно через `ARCH-REL-006` | Adapter, Cache-Aside, Eventual Consistency | Локальная проекция снижает нагрузку на предел 1С в 50 RPS и сохраняет доступность чтения при временном сбое; изменение полиса сверяется с источником без синхронного обновления пользовательской проекции | `ARCH-I-002`, `ARCH-I-006`, `ARCH-I-010` |
...
| `ARCH-REL-019` | CIAM, корпоративный IAM/SSO, Уведомления, внешний и внутренний шлюзы, цифровая, интеграционная и событийная платформы → платформа наблюдаемости | Асинхронная телеметрия и аудит, OpenTelemetry/OTLP | Centralized Observability, Correlation ID, Security Audit Trail | Общий trace/correlation ID связывает запросы, сообщения, доставку уведомлений и legacy-вызовы; события IAM и шлюзов обеспечивают аудит входов и изменений доступа | `ARCH-I-009`, `ARCH-I-011` |
| `ARCH-REL-020` | Веб-портал → Internal Gateway/Access Proxy: API-запросы врачей и администраторов | Синхронно, HTTPS/REST | Internal API Gateway, Access Proxy | Убирает прямые вызовы веб-портала в CRM и расписание и создаёт единый внутренний policy enforcement point | `ARCH-I-001`, `ARCH-I-011` |
| `ARCH-REL-021` | Лаборант → Internal Gateway/Access Proxy: доступ к интерфейсу и API LIS | Синхронно, HTTPS | Identity-Aware Proxy, Zero Trust Access | LIS не публикуется напрямую даже во внутренней сети; каждый сеанс проходит проверку идентичности и политики доступа | `ARCH-I-011` |
| `ARCH-REL-022` | Internal Gateway/Access Proxy → корпоративный IAM/SSO: вход и проверка сотрудника | Синхронно, OIDC/JWKS или SAML 2.0 | Enterprise SSO, Token Validation, MFA | Централизует аутентификацию врачей и лаборантов, блокировку учётных записей и обязательную MFA | `ARCH-I-011` |
| `ARCH-REL-023` | Internal Gateway/Access Proxy → Расписание: управление слотами и записями | Синхронно, REST | Gateway Routing, RBAC, Least Privilege | Врачи получают только разрешённые операции, а расписание не доступно напрямую из пользовательского сегмента | `ARCH-I-001`, `ARCH-I-005`, `ARCH-I-011` |
| `ARCH-REL-024` | Internal Gateway/Access Proxy → CRM: карточки и результаты пациентов | Синхронно, REST | Gateway Routing, RBAC, Policy Enforcement Point | Централизованная авторизация ограничивает доступ врача данными его пациентов и предотвращает обход политик CRM | `ARCH-I-001`, `ARCH-I-011` |
| `ARCH-REL-025` | Internal Gateway/Access Proxy → LIS: лабораторные результаты | Синхронно, HTTPS/REST | Access Proxy, RBAC, Least Privilege | Лаборант получает доступ только к функциям своей роли; исходная LIS остаётся внутренней системой без прямой пользовательской экспозиции | `ARCH-I-007`, `ARCH-I-011` |

## Как комбинируются архитектурные стили

1. **API-led integration** разделён на два периметра: Mobile App → External API Gateway → BFF для пациентов и Web Portal/LIS → Internal Gateway для сотрудников. У шлюзов разные политики безопасности, лимиты и SLA.
...
6. **Разделение Customer IAM и Workforce IAM** изолирует публичные учётные записи пациентов от корпоративных идентичностей сотрудников. Internal Gateway проверяет SSO-токены и применяет RBAC перед маршрутизацией к расписанию, CRM и LIS.
7. **Миграция LIS → CRM** выполняется как Strangler-переход: целевой поток идёт `LIS → интеграционная платформа → CRM`, а прежний прямой канал работает только во время параллельной сверки. После подтверждения полноты, порядка, идемпотентности и возможности replay прямой канал отключается; двойная постоянная запись в CRM не является целевой архитектурой.
...
| Безопасность медицинских данных | Раздельные External и Internal Gateway, OIDC/OAuth 2.1 для пациентов, корпоративный SSO/MFA и RBAC для сотрудников, отсутствие прямого пользовательского доступа к внутренним системам |
