...
| `ARCH-S-002` | IAM | Аутентификация и выдача токенов по OAuth 2.1 / OIDC | `ARCH-I-011` |
...
| `ARCH-REL-003` | API Gateway/WAF → IAM: проверка access token | Синхронно по JWT/JWKS с локальным кешированием ключей | Token Validation, Cache-Aside | Локальная криптографическая проверка JWT не создаёт сетевой вызов к IAM на каждый запрос и снижает риск нового узкого места | `ARCH-I-002`, `ARCH-I-005`, `ARCH-I-011` |
...
| `ARCH-REL-019` | API Gateway, цифровая, интеграционная и событийная платформы → платформа наблюдаемости | Асинхронная телеметрия, OpenTelemetry/OTLP | Centralized Observability, Correlation ID | Общий trace/correlation ID связывает мобильный запрос, сообщения и legacy-вызовы и сокращает поиск причины сбоя | `ARCH-I-009` |
...
| Безопасность медицинских данных | OIDC/OAuth 2.1, WAF, проверка токена на границе и отсутствие прямого доступа к внутренним системам |
